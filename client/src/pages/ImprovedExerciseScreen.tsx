...
import { IdentifyActivity } from "@/components/IdentifyActivity";
import { PositionActivity } from "@/components/PositionActivity";
...
import { generateActivities, Activity } from "@shared/musicNotes";

const TOTAL_QUESTIONS = 20;

export default function ImprovedExerciseScreen() {
  ...
  useEffect(() => {
    const initSession = async () => {
      try {
        ...
        setActivities(generateActivities(TOTAL_QUESTIONS));
      } catch (err) {
  ...
  const handleAnswerSubmit = async (answer: string) => {
    if (!sessionId || !currentActivity) return;

    setIsLoading(true);
    try {
      const result = await submitAnswerMutation.mutateAsync({
        sessionId,
        questionNumber: currentActivity.questionNumber,
        correctNote: currentActivity.note,
      ...
      setTimeout(() => {
        if (currentActivityIndex >= TOTAL_QUESTIONS - 1) {
  ...
  if (!sessionId || activities.length === 0) {
    return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 py-8 px-4">
      <div className="max-w-2xl mx-auto space-y-6">
        <Card className="shadow-lg">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="text-right">
                ...
                <p className="text-2xl font-bold">
                  {currentActivityIndex + 1}/{TOTAL_QUESTIONS}
                </p>
              </div>
            </div>
          </CardHeader>
          ...
          <CardContent className="pt-6">
            <Progress
              value={((currentActivityIndex + 1) / TOTAL_QUESTIONS) * 100}
              className="h-2"
            />
          </CardContent>
        </Card>
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-indigo-600">
                  {currentActivityIndex > 0
                    ? Math.round((score / currentActivityIndex) * 100)
                    : 0}
                  %
                </p>
              </div>
